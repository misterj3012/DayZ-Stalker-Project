class ViewInventoryHandlerBase
{
    ...
    bool PlayAnimationCondition( bool canclecondition = false )
    {
        if ( !m_IsViewingInventorySynced || m_Player.IsRestrained() || m_Player.IsRestrainStarted() )
        {
            return false;
        }
        if ( !canclecondition && m_Player.GetCommandModifier_Action() )
        {
            return false;
        }
        if ( canclecondition && !ViewInventoyCommandActionCallback.Cast( m_Player.GetCommandModifier_Action() ) )
        {
            return false;
        }
    ...
    void OnUpdate( int pCurrentCommandID, float pDt )
    {
        if ( pCurrentCommandID == DayZPlayerConstants.COMMANDID_MOVE )
        {
            if ( m_Callback )
            {
                // for some reason the cancle condition of the command callback is not realiable enough..
