...
modded class ActionTestBloodSelf
{
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (GameConstants.BLOODTESTKIT_YIELD_REPORT)
		{
			PlayerBase player = action_data.m_Player;

			AMS_TestKitReport report = AMS_TestKitReport.Cast(player.GetInventory().CreateInInventory("AMS_TestKitReport"));
			if (!report)
			{
				report = AMS_TestKitReport.Cast(player.SpawnEntityOnGroundOnCursorDir("AMS_TestKitReport", 0.5));
			}
