class EditorAnomalySpawnerFile : EditorFileType
{
	...
	{
		EditorLog.Trace("EditorAnomalySpawnerFile::Export");
		AnomalyArea export_data = new AnomalyArea();
		export_data.Anomalies = {};
		foreach(EditorObjectData object_data : data.EditorObjects)
		{
			if (object_data.Type.Contains("Anomaly"))
			{
				AnomalyObjects spawn_object = new AnomalyObjects();
				...
				export_data.Anomalies.Insert(spawn_object);
			}
		}
		FileSerializer file_serializer = new FileSerializer();
	...
	{
		EditorLog.Trace("EditorAnomalySpawnerFile::Import");
		...
		{
			JsonFileLoader<AnomalyArea>.JsonLoadFile(file, import_data);
		}
		if (import_data)
		{
			foreach(AnomalyObjects scene_object : import_data.Anomalies)
			{
