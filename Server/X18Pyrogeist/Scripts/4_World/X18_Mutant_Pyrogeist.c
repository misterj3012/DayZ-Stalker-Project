modded class X18_Mutant_Pyrogeist
{
	...
	override void EEKilled(Object killer)
	{
		...
		GetGame().CreateObject("X18_PyrogeistDeath_Fx", death_position, false);
		GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(Delete, 100, false);
	}
	override void EEHitBy(TotalDamageResult damageResult, int damageType, EntityAI source, int component, string dmgZone, string ammo, vector modelPos, float speedCoef)
	{
		if (!m_hit)
			m_hit = X18_PyrogeistHit_Fx.Cast(GetGame().CreateObject("X18_PyrogeistHit_Fx", this.GetPosition(), false));
	...
	void Delete()
	{
		...
		{
			SpawnLoot(50, "X18_Artefact_Fireball");
			loot_spawned = true;
		}
	...
	void TargetSearch()
	{
		...
		for (int p = 0; p < players.Count(); p++)
		{
			...
			if (!IsOnCooldown && random_player.IsAlive())
			{
				...
				GetGame().CreateObject("X18_Pyrogeist", anomaly_spawn_position, false);
