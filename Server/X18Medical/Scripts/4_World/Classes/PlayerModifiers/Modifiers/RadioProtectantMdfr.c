class RadioProtectantMdfr : ModifierBase
{
	...
	override void Init()
	{
		...
		m_ID = X18MedicalModifiers.MDF_RADIOPROTECTANT;
		m_TickIntervalInactive = 1;
		m_TickIntervalActive = 1;
		m_IsPersistent = true;
		m_EffectLifetime = MedicalConfig.B190Duration;
	}
	...
	{
		float m_Radioprotectant = player.GetSingleAgentCount(X18MedicalAgents.RADIOPROTECTANT);
		if (player.IsUnconscious())
			return;
		if (m_Radioprotectant >= 5 && (0.15 > Math.RandomFloatInclusive(0, 1)))
		{
		...
		if (m_Radioprotectant >= 7 && (0.2 > Math.RandomFloatInclusive(0, 1)))
